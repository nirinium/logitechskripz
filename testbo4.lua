...
function OnEvent(event, arg)
	...
	if(event == "MOUSE_BUTTON_PRESSED" and arg == 2)then--AUTO HOLD BREATH
	...
	if(event == "MOUSE_BUTTON_RELEASED" and arg == 2)then --AUTO HOLD BREATH
...
function reloading(x)--自动换弹模块
	if auto_reloading then
		--OutputLogMessage("%d\n",x) OUTPUTS RELOADING CONFIRMATION
		if IsM16 then
